...
def main():
    ...
    # TODO(dmsimard): Make this an argument of the script
    WHITELIST = [
        'current-tripleo',
        'current-tripleo-rdo',
    ...
    try:
        ...
        # This should always run regardless of dry-run or not
        ...
        raise e
    log.info("%s total image tags found." % len(istags['items']))

    log.info("%s tag names whitelisted: %s" % (
        len(WHITELIST), ", ".join(WHITELIST))
    )
    # We need to iterate a first time across the list of all available tags
    # in order to retrieve image tags which might match the same image.
    for istag in istags['items']:
        name, tag = istag['metadata']['name'].split(':')

        for tag in image['status']['tags']:
            if tag['tag'] in WHITELIST:
                # 3b1e1c01-c7e6-11e7-bb2f-fa163e7f8b33
    ...
    for istag in istags['items']:
        ...
        if tag in WHITELIST:
            ...
            # Verify if this is a tag matching a whitelisted tag
